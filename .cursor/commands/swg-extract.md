...
- **필수: 중복 제거**: 동일한 파일/클래스는 한 번만 카운트하고 출력하세요. "중복 포함" 같은 표현은 사용하지 마세요.
...
**⚠️ 필수 지시사항:**
1. **파일 링크는 결과 설명에 바로 포함**: 별도의 "분석된 파일 링크" 섹션을 만들지 말고, 본문에 바로 링크 포함
2. **중복 제거**: 동일한 파일/클래스는 한 번만 카운트하고 나열. "중복 포함", "실제 고유 클래스" 같은 표현 금지
